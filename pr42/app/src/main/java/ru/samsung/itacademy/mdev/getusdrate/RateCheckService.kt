...
class RateCheckService : Service() {
    ...
    val rateCheckRunnable: Runnable = Runnable {
        ...
        if (rateCheckAttempt > RATE_CHECK_ATTEMPTS_MAX) {
            Toast.makeText(applicationContext, "Достигнуто максимальное количество запросов", Toast.LENGTH_LONG).show()
            stopSelf()
            Log.d(TAG, "Достигнуто максимальное количество запросов")
            return@Runnable
        }
        requestAndCheckRate()
    }
